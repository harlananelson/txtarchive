...
def split_file(file_path, max_chars=MAX_CHARS, output_dir=OUTPUT_DIR):
    ...
    file_name = Path(file_path).stem  # e.g., 'cohort' from 'cohort.py'
    file_ext = Path(file_path).suffix  # e.g., '.py'
    ...
    if total_chars <= max_chars:
        ...
        print(f"Copied {file_name}{file_ext} ({total_chars} chars)")
        return
    ...
    for line in lines:
        ...
        if current_chars + line_chars > max_chars and current_chunk:
            ...
            print(f"Wrote {output_path} ({current_chars} chars)")
            current_chunk = []
            current_chars = 0
            part_num += 1
